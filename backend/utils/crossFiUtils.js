...
async function crossFiAccountDetails(userAddress) {
  try {
      ...
      const res = await axios.get(
          `https://cosmos-api.testnet.ms/cosmos/bank/v1beta1/balances/${userAddress}`
        );
        
        // const balance = res.data.coins[0].amount;
        
        res.data.balances.forEach((balance) => {
            balance.amount = (Number(balance.amount) / 1e18).toString(); 
        });
        console.log("crossFiAccont k andr  responsne",res.data);
        console.log("crossFiAccont k andr user addres ",userAddress);
    ...
    const transactionRes = await axios.get(
      `https://test.xfiscan.com/api/1.0/txs?address=${userAddress}`
    );
